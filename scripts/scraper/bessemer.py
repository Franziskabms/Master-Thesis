...
# Wayback CDX API
CDX_QUERIES = [
    "http://web.archive.org/cdx/search/cdx?url=bvp.com/atlas/*&output=json&collapse=urlkey&fl=original&filter=statuscode:200",
    "http://web.archive.org/cdx/search/cdx?url=www.bvp.com/atlas/*&output=json&collapse=urlkey&fl=original&filter=statuscode:200",
]
...
def collect_urls_from_cdx() -> set[str]:
    """Uses the Wayback CDX API to collect all Atlas article URLs"""
    urls = set()
    ...
    for cdx_url in CDX_QUERIES:
        ...
        try:
            ...
            for row in data[1:]:
                ...
                # Only article URLs (no category/home pages)
                parsed = urlparse(url)
                path = parsed.path.rstrip("/")

                # Remove UTM parameters and other tracking parameters
                clean_url = f"https://www.bvp.com{path}"

                if not parsed.path.startswith("/atlas/"):
                    continue
    ...
    print(f"  → {len(urls)} unique Atlas-URLs found")
    return urls
