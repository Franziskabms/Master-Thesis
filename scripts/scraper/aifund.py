...
OUT_JSONL_EN = OUT_DIR / "documents_en.jsonl"       # English articles
OUT_JSONL_DE = OUT_DIR / "documents_de.jsonl"       # German articles
OUT_JSONL_OTHER = OUT_DIR / "documents_other.jsonl" # Other languages
...
def extract_doc(url: str, html: str) -> dict:
    ...
    # Language from HTML tag as fallback
    if not language:
        soup = BeautifulSoup(html, "html.parser")
...
def run_scrape(max_docs: int = 500, apply_keyword_filter: bool = True):
    ...
    for i, url in enumerate(urls):
        ...
        print(f"  ✓ saved [{lang}] | {doc['char_count']} chars | {doc['published_time']}")
    ...
    for label, out_file in [("EN", OUT_JSONL_EN), ("DE", OUT_JSONL_DE), ("Other", OUT_JSONL_OTHER)]:
        if out_file.exists():
            ...
            print(f"  {label}: {count} documents → {out_file.name}")
