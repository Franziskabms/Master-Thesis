...
# Wayback CDX API
CDX_QUERIES = [
    "http://web.archive.org/cdx/search/cdx?url=accel.com/news/*&output=json&collapse=urlkey&fl=original&filter=statuscode:200",
    "http://web.archive.org/cdx/search/cdx?url=www.accel.com/news/*&output=json&collapse=urlkey&fl=original&filter=statuscode:200",
]

# Direct category pages for crawling
CATEGORY_PAGES = [
    "https://www.accel.com/news",
]
    ...
    for cdx_url in CDX_QUERIES:
        ...
        try:
            ...
            for row in data[1:]:
                ...
                # Only articles (no .html, .php, .gif, etc.)
                if any(path.endswith(ext) for ext in [".html", ".php", ".gif", ".jpg", ".png", ".pdf"]):
                    continue

                # Only /news/ subpaths with real article names
                if len(path.split("/")) < 3:
                    continue

                clean_url = f"https://www.accel.com{path}"
                urls.add(clean_url)

        except Exception as e:
            print(f"  → CDX Error: {e}")

    print(f"  → {len(urls)} URLs from Wayback CDX")
    return urls
...
def collect_urls_from_category_pages() -> set[str]:
    ...
    for base_url in CATEGORY_PAGES:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if (parsed.netloc.endswith(base_domain)
                    ...
                    and not any(x in parsed.path for x in [
                        "/team/", "/companies/", "/contact", "/spotlight-on/",
                        "/founder-factory/", "/globalscape/", "/news-eu", "/news-in",
                        "/news-featured",
                    ])
        ...
        urls.update(found)
        print(f"  → {len(found)} Links found on category page")
...
def build_url_list() -> list[str]:
    ...
    print("[1/2] Wayback CDX API...")
    all_urls.update(collect_urls_from_cdx())
