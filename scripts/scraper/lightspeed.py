...
OUT_DIR = Path("lightspeed_corpus")
...
# Sitemap index URL
SITEMAP_INDEX = "https://lsvp.com/sitemap_index.xml"

# Whitelist: only scrape relevant partial sitemaps
RELEVANT_SITEMAPS = {
    "https://lsvp.com/post-sitemap.xml",
    "https://lsvp.com/newsletter-sitemap.xml",
}
...
SKIP_PATH_SEGMENTS = [
    ...
    "/wp-content/", "/event/", "/gallery/", "/firm/",
]

# File extensions to skip
SKIP_EXTENSIONS = (
    ".jpg", ".jpeg", ".png", ".gif", ".svg",
    ".pdf", ".mp4", ".mp3", ".webp", ".zip"
)
...
def collect_urls_from_sitemaps() -> set[str]:
    ...
    if not index_xml:
        print("  [Warning] Sitemap index unavailable")
        return urls
...
def build_url_list() -> list[str]:
    """Collects all candidate URLs from sitemaps, then filters out non-article pages."""
    ...
    # Filter out non-article pages and media files
    all_urls = {
        u for u in all_urls
        if not any(seg in u for seg in SKIP_PATH_SEGMENTS)
        and not u.split("?")[0].lower().endswith(SKIP_EXTENSIONS)
    }
...
def run_scrape(max_docs: int = 1000, apply_keyword_filter: bool = True):
    """Main scraping loop.
