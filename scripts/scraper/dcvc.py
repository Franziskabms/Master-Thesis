...
# Wayback CDX API
CDX_QUERIES = [
    "http://web.archive.org/cdx/search/cdx?url=dcvc.com/news-insights/*&output=json&collapse=urlkey&fl=original&filter=statuscode:200",
    "http://web.archive.org/cdx/search/cdx?url=www.dcvc.com/news-insights/*&output=json&collapse=urlkey&fl=original&filter=statuscode:200",
]
    ...
    for cdx_url in CDX_QUERIES:
        ...
        try:
            ...
            for row in data[1:]:
                ...
                if not parsed.path.startswith("/news-insights/"):
                    continue

                # Filter out archive pages and pagination
                if any(x in path for x in ["/archive", "/page"]):
                    continue
                ...
                clean_url = f"https://www.dcvc.com{path}"
                urls.add(clean_url)
